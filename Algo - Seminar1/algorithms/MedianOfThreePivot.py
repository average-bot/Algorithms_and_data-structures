...
class MedianOfThreePivot:
    ...
    def partition(self, array, startIndex, endIndex):
        ...
        meanArray = [array[middleIndex], array[startIndex], array[endIndex]]
        meanArray.remove(min(meanArray))
        meanArray.remove(max(meanArray))
        meanValue = meanArray[0]
        meanIndex = array.index(meanValue)

        pivot = meanValue
        array[meanIndex], array[endIndex] = array[endIndex], array[meanIndex]
